...
function getUsers() {
	var col = Alloy.Collections.tasks;

	col.fetch({
		success : function(model, response) {
			response.forEach(function(task) {
			    var latitude = task.location.latitude;
			    var longitude = task.location.longitude;
			    var title = task.title;
			    var image = task.user.imageurl;
...
function dataTransform(model) {
	...
	m.image = m.user.imageurl;
	m.name = m.user.name;
	m.date = dateTransform(m.date);
...
function dateTransform(date){
	...
	var difMonth = month.toString().length;
	...
	var difDay = day.toString().length;
...
function taskClicked(){
	...
	var model = Alloy.Collections.tasks.get(this.modelId);
	//var model = Alloy.Collections.cars.get(_event.rowData.Id);
	
	//create the controller and pass the model to it
	var taskDescController = Alloy.createController("taskDesc", {
		data : model
	});
	
	taskDescController.taskDesc.open();
	$.discover.close();
}